...
- (void)viewInit {
  ...
  descriptionLabel.text = @"This is the description label that's longer and wraps to multiple lines so that you can see how the imageView is able to vertically center on the two labels grouped together in a sublayout.";
  ...
  self.layout = [YOLayout layoutWithLayoutBlock:^CGSize(YOLayout *layout, CGSize size) {
    ...
    CGRect labelsRect = [layout setFrame:CGRectMake(x, y, size.width - x - 10, CGFLOAT_MAX) view:labelLayout options:YOLayoutOptionsSizeToFitVertical | YOLayoutOptionsRTLIfNeeded];

    // Image view should be vertically centered in the the labelLayout's height
    [layout setSize:imageView.image.size inRect:CGRectMake(10, labelsRect.origin.y, imageView.image.size.width, labelsRect.size.height) view:imageView options:YOLayoutOptionsAlignCenterVertical | YOLayoutOptionsRTLIfNeeded];
