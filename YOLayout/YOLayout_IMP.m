...
- (CGRect)setSize:(CGSize)size inRect:(CGRect)inRect view:(id)view options:(YOLayoutOptions)options {
  ...
  if (!_sizing) {
    ...
    CGRect flippedFrame = frame;
    if (((options & YOLayoutOptionsRTLIfNeeded) != 0) && ([UIApplication sharedApplication].userInterfaceLayoutDirection == UIUserInterfaceLayoutDirectionRightToLeft)) {
    ...
    if (!YOCGRectIsEqual(self.frame, CGRectZero)) {
      flippedFrame.origin.x += self.frame.origin.x;
      flippedFrame.origin.y += self.frame.origin.y;
    }
    
    [view setFrame:flippedFrame];

    // Since we are applying the frame, the subview will need to re-layout
    // QQ: Is this necessary? Doesn't UIView already do this when you setFrame?
